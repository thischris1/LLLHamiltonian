...
#ifndef LLLhamInplaneMagImpRectg_HPP
#define LLLhamInplaneMagImpRectg_HPP
...
class LLLhamInplaneMagImpRectg : public LLLhamiltonian
{
private:
  double MIstrength;     // 
  double IMIstrength;     // 
...
protected:
  inline virtual int inhmgtyGeometry(int j);
  
public:
  LLLhamInplaneMagImpRectg(yosBasis *new_basis,int new_type,
		      double bli_new,double a_new, double b_new,
		      double new_ZeemanStrength,
		      double new_MIstrength,
		      double new_IMIstrength);

  ~LLLhamInplaneMagImpRectg();
